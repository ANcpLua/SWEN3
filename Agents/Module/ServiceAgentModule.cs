...
public static class ServiceCollectionExtensions
{
    public static void AddServiceAgents(this IServiceCollection services, IConfiguration configuration)
    {
        ...
        {
            Console.WriteLine("ElasticSearch configuration section is missing.");
        }
        else
        {
            Console.WriteLine($"ElasticSearch Url: {elasticSearchConfig.Url}");
            Console.WriteLine($"ElasticSearch DefaultIndex: {elasticSearchConfig.DefaultIndex}");
        }
        ...
        var rabbitMqConfig = configuration.GetSection("RabbitMQ").Get<RabbitMqConfig>() 
                             ?? throw new InvalidOperationException("RabbitMQ configuration is not provided or invalid");

        if (string.IsNullOrEmpty(rabbitMqConfig.Host) || rabbitMqConfig.Port == 0)
            throw new InvalidOperationException("RabbitMQ Host or Port is not configured");
